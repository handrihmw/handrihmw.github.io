...
gulp.task('scss', function () {
  return gulp.src([paths.src.scss + '/palagan/**/*.scss', paths.src.scss + '/palagan.scss']).pipe(wait(500)).pipe(sourcemaps.init()).pipe(sass().on('error', sass.logError)).pipe(autoprefixer({
    overrideBrowserslist: ['> 1%']
  })).pipe(sourcemaps.write('.')).pipe(gulp.dest(paths.temp.css)).pipe(browserSync.stream());
});
gulp.task('index', function () {
  return gulp.src([paths.src.base + '**.html']).pipe(fileinclude({
    prefix: '@@',
    basepath: './src/partials/',
    context: {
      environment: 'development'
    }
  })).pipe(gulp.dest(paths.temp.base)).pipe(browserSync.stream());
});
gulp.task('html', function () {
  return gulp.src([paths.src.html]).pipe(fileinclude({
    prefix: '@@',
    basepath: './src/partials/',
    context: {
      environment: 'development'
    }
  })).pipe(gulp.dest(paths.temp.html)).pipe(browserSync.stream());
});
...
gulp.task('serve', gulp.series('scss', 'html', 'index', 'assets', 'vendor', function () {
  ...
  gulp.watch([paths.src.scss + '/palagan/**/*.scss', paths.src.scss + '/palagan.scss'], gulp.series('scss'));
...
gulp.task('beautify:css', function () {
  return gulp.src([paths.dev.css + '/palagan.css']).pipe(cssbeautify()).pipe(gulp.dest(paths.dev.css));
}); // Minify CSS

gulp.task('minify:css', function () {
  return gulp.src([paths.dist.css + '/palagan.css']).pipe(cleanCss()).pipe(gulp.dest(paths.dist.css));
}); // Minify Html

gulp.task('minify:html', function () {
  return gulp.src([paths.dist.html + '/**/*.html']).pipe(htmlmin({
...
gulp.task('minify:html:index', function () {
  return gulp.src([paths.dist.base + '*.html']).pipe(htmlmin({
    collapseWhitespace: true
  })).pipe(fileinclude({
    prefix: '@@',
    basepath: './src/partials/',
...
gulp.task('copy:dist:css', function () {
  return gulp.src([paths.src.scss + '/palagan/**/*.scss', paths.src.scss + '/palagan.scss']).pipe(wait(500)).pipe(sourcemaps.init()).pipe(sass().on('error', sass.logError)).pipe(autoprefixer({
    overrideBrowserslist: ['> 1%']
  })).pipe(sourcemaps.write('.')).pipe(gulp.dest(paths.dist.css));
});
gulp.task('copy:dev:css', function () {
  return gulp.src([paths.src.scss + '/palagan/**/*.scss', paths.src.scss + '/palagan.scss']).pipe(wait(500)).pipe(sourcemaps.init()).pipe(sass().on('error', sass.logError)).pipe(autoprefixer({
    overrideBrowserslist: ['> 1%']
  })).pipe(sourcemaps.write('.')).pipe(gulp.dest(paths.dev.css));
}); // Copy Html
...
gulp.task('build:dist', gulp.series('clean:dist', 'copy:dist:css', 'copy:dist:html', 'copy:dist:html:index', 'copy:dist:assets', 'minify:css', 'minify:html', 'minify:html:index', 'copy:dist:vendor')); // Default
