...
const paths = {
    ...
    twig: {
        html: './src/partials/components/twig'
    }
};

// Twig
gulp.task('templates', function() {
    return gulp.src('src/partials/components/card.html')
        .pipe(fileinclude({
            prefix: '@@',
            basepath: './src/partials/components/twig/',
            context: {
                environment: 'development'
            }
        }))
        .pipe(twig())
        .pipe(gulp.dest(paths.twig.html))
        .pipe(browserSync.stream());
});

// Compile SCSS
gulp.task('scss', function () {
    return gulp.src([paths.src.scss + '/palagan/**/*.scss', paths.src.scss + '/palagan.scss'])
        .pipe(wait(500))
